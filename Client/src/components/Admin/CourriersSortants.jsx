// CourriersSortants.jsx
import { useState, useEffect, useRef, useCallback } from "react";
import { useNavigate } from "react-router-dom";
import {
  Send, Plus, Search, Eye, Pencil, Trash2,
        ...
        {!imgFail ? (
          <>
            ...
            <div style={{ padding:"12px 18px",borderTop:`1px solid ${SS.border}`,display:"flex",gap:8,flexWrap:"wrap" }}>
              {[
                { label:"Visualiser", icon:<Eye size={13}/>, fn:()=>setViewer("image"), primary:true },
                { label:"Télécharger", icon:<Download size={13}/>, fn:downloadFile },
                { label:"Ouvrir", icon:<ExternalLink size={13}/>, fn:()=>window.open(scanUrl,"_blank") },
              ].map((b,i)=>(
                <button key={i} onClick={b.fn} style={{ display:"inline-flex",alignItems:"center",gap:6,padding:"7px 14px",borderRadius:8,border:`1px solid ${b.primary?SS.accentBorder:SS.border}`,background:b.primary?SS.accentBg:"transparent",color:b.primary?SS.accent:SS.textSec,fontSize:12,fontWeight:600,cursor:"pointer" }}>
                  {b.icon}{b.label}
                </button>
              ))}
            </div>
...
// ════════════════════════════════════════════════════════
//  FORMULAIRE COURRIER SORTANT
// ════════════════════════════════════════════════════════
const EMPTY_FORM = () => ({
  origine:"", objet:"", destinataire:"", date_sortie:today(), scan:null
});
...
const FormCourrier = ({ courrier, onSave, onCancel, SS }) => {
  const [form,    setForm]    = useState(courrier ? {
    ...
    date_sortie:  courrier.date_sortie  || today(),
    scan:         null,
  } : EMPTY_FORM());
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.origine||!form.objet||!form.destinataire) {
      setError("Origine, Objet et Destinataire sont obligatoires."); return;
    }
    ...
    try {
      ...
      const url    = courrier ? CONFIG.API_COURRIER_SORTANT_DETAIL(courrier.id) : CONFIG.API_COURRIERS_SORTANTS;
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display:"flex",flexDirection:"column",gap:20 }}>
      ...
      {/* Ligne 1 : Origine + Date sortie */}
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16 }}>
        <div><FieldLabel icon={Building2} SS={SS}>Origine</FieldLabel><Input SS={SS} type="text" placeholder="Ex: Direction Générale" value={form.origine} onChange={e=>set("origine",e.target.value)} required/></div>
        <div><FieldLabel icon={Calendar} SS={SS}>Date de sortie</FieldLabel><Input SS={SS} type="date" value={form.date_sortie} onChange={e=>set("date_sortie",e.target.value)} required/></div>
      </div>

      {/* Destinataire */}
      <div>
        <FieldLabel icon={User} SS={SS}>Nom du destinataire</FieldLabel>
        <Input SS={SS} type="text" placeholder="Nom complet du destinataire ou entité" value={form.destinataire} onChange={e=>set("destinataire",e.target.value)} required/>
      </div>

      {/* Objet */}
      <div>
        <FieldLabel icon={AlignLeft} SS={SS}>Objet</FieldLabel>
        <Textarea SS={SS} placeholder="Objet du courrier sortant..." value={form.objet} onChange={e=>set("objet",e.target.value)} required/>
      </div>
...
const DetailCourrier = ({ courrier, onClose, onEdit, onDeleted, onToast, SS }) => {
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Supprimer ${courrier.numero_sortie} ?`)) return;
    try {
      const res = await fetch(CONFIG.API_COURRIER_SORTANT_DETAIL(courrier.id), { method:"DELETE",headers:authHdr() });
  ...
  const handlePrint = () => {
    ...
    w.document.write(buildPrintHTML(courrier));
        ...
        <div style={{ display:"flex",alignItems:"center",gap:14 }}>
          ...
          <div>
            <div style={{ fontSize:10,color:SS.textDim,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:3 }}>
              Fiche courrier sortant
            </div>
            <div style={{ fontSize:22,fontWeight:900,color:SS.accent,letterSpacing:"0.04em" }}>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:16 }}>
          ...
          <div style={{ background:SS.card,border:`1px solid ${SS.accentBorder}`,borderRadius:14,padding:"20px 22px",position:"relative",overflow:"hidden" }}>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginTop:6 }}>
              {[
                { label:"Date de sortie",    value:fmt(courrier.date_sortie), hi:false },
                { label:"Origine",           value:courrier.origine,          hi:true  },
                { label:"Numéro de sortie",  value:courrier.numero_sortie,    hi:false },
                { label:"Destinataire",      value:courrier.destinataire,     hi:true  },
              ].map(({label,value,hi})=>(
                <div key={label}>
...
const CourriersSortants = () => {
  ...
  const load = useCallback(async (p=1, q="") => {
    ...
    try {
      ...
      const res  = await fetch(`${CONFIG.API_COURRIERS_SORTANTS}?${params}`, { headers:authHdr() });
      const data = await res.json();
  ...
  const openDetail = async (courrier) => {
    try {
      const res  = await fetch(CONFIG.API_COURRIER_SORTANT_DETAIL(courrier.id), { headers:authHdr() });
      const data = await res.json();
  ...
  const handleSaved = (data) => {
    if (formMode==="create") {
      ...
      showToast(`Courrier ${data.numero_sortie} créé.`);
    } else {
  ...
  const breadcrumbs = [
    ...
    { label:"Sortants", action:()=>{ setView("list"); setSelected(null); } },
    ...(view==="detail"&&selected ? [{ label:selected.numero_sortie, action:null }] : []),
      ...
      {view === "list" && (
        <div style={{ animation:"pageIn 0.2s ease" }}>
          ...
          <div style={{ display:"flex",alignItems:"flex-end",justifyContent:"space-between",flexWrap:"wrap",gap:16,marginBottom:24 }}>
            <div>
              <h1 style={{ fontSize:28,fontWeight:900,color:SS.text,margin:"0 0 6px",letterSpacing:"-0.02em" }}>Courriers Sortants</h1>
          ...
          <div style={{ display:"flex",gap:10,marginBottom:22,maxWidth:520 }}>
            ...
            {search && (
              <button onClick={()=>setSearch("")} style={{ padding:"9px 16px",borderRadius:10,border:`1px solid ${SS.border}`,background:"transparent",color:SS.textSec,cursor:"pointer",display:"flex",alignItems:"center",gap:6,fontSize:12,fontWeight:600 }}>
                <X size={13}/> Effacer
              </button>
            )}
          </div>
          ...
          {loading && !search ? (
            ...
            <div style={{ textAlign:"center",padding:"80px 0",background:SS.card,borderRadius:16,border:`1px solid ${SS.border}` }}>
              <Send size={40} color={SS.textDim} style={{ marginBottom:16 }}/>
              <div style={{ fontSize:15,fontWeight:700,color:SS.textSec,marginBottom:8 }}>
                {search?`Aucun résultat pour "${search}"`:"Aucun courrier sortant enregistré"}
              </div>
                ...
                <table style={{ width:"100%",borderCollapse:"collapse",fontSize:13 }}>
                  <thead>
                    <tr style={{ background:SS.surface,borderBottom:`1px solid ${SS.border}` }}>
                      {["N° de sortie","Date sortie","Origine","Destinataire","Objet","Scan","Actions"].map(h=>(
                  ...
                  <tbody>
                    {courriers.map((c,i)=>(
                      ...
                      >
                        <td style={{ padding:"13px 16px",fontWeight:800,color:SS.accent,whiteSpace:"nowrap",fontSize:13 }}>{c.numero_sortie}</td>
                        <td style={{ padding:"13px 16px",color:SS.textSec,whiteSpace:"nowrap",fontSize:12 }}>{fmt(c.date_sortie)}</td>
                        <td style={{ padding:"13px 16px",fontWeight:600,color:SS.text }}>{c.origine}</td>
                        <td style={{ padding:"13px 16px",color:SS.textSec,fontSize:12 }}>{c.destinataire}</td>
                        <td style={{ padding:"13px 16px",color:SS.textSec,maxWidth:180,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontSize:12 }}>{c.objet}</td>
        ...
        <div style={{ animation:"pageIn 0.2s ease" }}>
          <div style={{ display:"flex",alignItems:"center",gap:16,marginBottom:28 }}>
            <button onClick={()=>setView(selected&&formMode==="edit"?"detail":"list")} style={{ display:"flex",alignItems:"center",gap:8,padding:"9px 16px",borderRadius:10,border:`1px solid ${SS.border}`,background:"transparent",color:SS.textSec,fontSize:13,fontWeight:600,cursor:"pointer",transition:"all 0.15s" }}
              ...
              onMouseLeave={e=>{ e.currentTarget.style.borderColor=SS.border; e.currentTarget.style.color=SS.textSec; }}
            >
              <ArrowLeft size={15}/> {selected&&formMode==="edit"?"Retour à la fiche":"Retour à la liste"}
            </button>
            <div>
              <div style={{ fontSize:10,color:SS.textDim,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:3 }}>
                {formMode==="create"?"Nouveau courrier sortant":"Modifier le courrier"}
              </div>
              <h2 style={{ fontSize:20,fontWeight:800,color:SS.text,margin:0 }}>
                {formMode==="create"?"Fiche de sortie":"Modifier — "+selected?.numero_sortie}
              </h2>
            </div>
          </div>
